...
daily_net_flow AS (
    ...
    JOIN query_4261812 yc ON erc20_sei.evt_Transfer.contract_address = yc.a_token
...
usd_value_per_symbol AS (
    SELECT
        ...
        COALESCE(SUM(CASE WHEN dp.symbol = 'sfrxUSD' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS sfrxusd_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'frxUSD' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS frxusd_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'WBTC' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS wbtc_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'Solvbtc' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS solvbtc_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'xSolvbtc' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS xsolvbtc_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'wstETH' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS wsteth_value,
        COALESCE(SUM(CASE WHEN dp.symbol = 'USDT0' THEN cb.sfastusd_cumulative_balance * dp.usd_price ELSE 0 END), 0) AS usdt0_value
